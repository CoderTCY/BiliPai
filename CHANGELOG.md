...
[添加] 搜索框、联想词和搜索历史提交时优先识别 BV 号、B 站链接及 b23.tv 短链并跳转站内页面；普通关键词仍在当前搜索页显示结果

[更改] 搜索发现默认按官方接口顺序展示推荐词、图标与直播标识，不再混入本地历史、关注 UP 或固定词；关闭个性化搜索推荐时改用公开热搜词，搜索发现显隐另有独立开关

[修复] 升级旧配置时分别保留个性化搜索推荐和搜索发现显隐的已存值，切换其中一个不再覆盖另一个
...
[添加] 搜索提交支持识别 BV 号、B 站页面及 b23.tv 短链并跳转对应原生页面；普通关键词与无法解析的短链仍在搜索页搜索
